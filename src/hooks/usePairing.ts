...
// Fallback polling apenas se Realtime não conectar
const POLL_FALLBACK_MS  = 4000
const REALTIME_TIMEOUT  = 5000  // aguarda 5s para WS conectar antes de ativar polling
...
export function usePairing(): UsePairingResult {
  ...
  const clearAll = () => {
    ...
    if (rtTimeoutRef.current) clearTimeout(rtTimeoutRef.current)
    if (wsRef.current) {
  ...
  const startPairing = useCallback(async () => {
    ...
    // ── FIX #4: Tenta Realtime primeiro ───────────────────────────────────
    const ws = createRealtimeChannel(
      'pair_tokens',
      `token=eq.${token.token}`,
      handleLinked
    )
    wsRef.current = ws

    // Fallback polling: ativa se o WS não conectar em 5s
    rtTimeoutRef.current = setTimeout(() => {
      const wsOk = wsRef.current?.readyState === WebSocket.OPEN
      if (!wsOk) {
        console.warn('[Pairing] Realtime não conectou — ativando polling fallback')
